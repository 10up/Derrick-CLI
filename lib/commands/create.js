...
module.exports = function() {
	...
	var schema = {
		properties: {
			...
			wordpress: {
				description: 'WordPress version to use (leave blank for latest)'
			}
		}
	};
		...
		return new NPromise( function( fulfill ) {
			...
			var vendorSchema = {
				properties: {
					...
					url: {
						description: "If this isn't located in the WordPress.org repository, remote location of an archive hosting the resource's code."
					},
					reference: {
						description: 'Commit or version reference for checkout. If omitted, the system will check out the current HEAD of the master branch or trunk.'
					}
				}
			};
		...
		return new NPromise( function( fulfill ) {
			...
			var devSchema = {
				properties: {
					...
					name: {
						description: 'The name of the resource. If not defined, the basename value of url will be used after removing any vcs-related endings.'
					},
					vcs: {
					...
					location: {
						description: 'Local new resource path. Defaults to the name property. Use this to change the default location of a resource (e.g. VIP themes should be installed in the /vip/ subdirectory inside /themes/). Use {name} as a placeholder for the default name (e.g. vip/{name}).'
					}
				}
			};
