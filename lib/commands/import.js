...
function ensureVagrantIsRunning( args ) {
	...
	function checkServerAvailability( vm ) {
		...
		return new NPromise( function ( fulfill, reject ) {
			net.createConnection( 239, 'moonshine.dev' ).on( 'connect', function () {
				fulfill( vm );
			} ).on( 'error', reject );
		} );
	}
...
function createProjectDirectory( args ) {
	...
	return new NPromise( function ( fulfill ) {
		...
		mkdir( project )
			.then( function () {
				return mkdir( path.join( project, 'config' ) );
			} )
			.then( function () {
				return mkdir( path.join( project, 'config/nginx' ) );
			} )
			.then( function() {
				return mkdir( path.join( project, 'config/nginx/sites' ) );
			} )
			.then( function() {
				return mkdir( path.join( project, 'config/nginx/conf.d' ) );
			} )
			.then( function () {
				fulfill( [vm, data] );
			}, _exit_error );
 ...
 * @returns {*}
 */
function installVendorResources( vendorResources, wpVersion, cache ) {
	return new NPromise( function ( fulfill, reject ) {
		var resourcePromises = [], x;
		...
		try {
			...
			for ( x = 0; x < vendorResources.length; x += 1 ) {
				var resource = resources.Resource.newFromData( vendorResources[x] ),
					resourcePath = path.join( cache, resource.path, resource.reference );
	...
	return new NPromise( function ( fulfill, reject ) {
		try {
			NPromise.all( [
				( function() {
					if ( data.database ) {
						return require( '../commands' ).import_db( data.name, data.database );
					} else {
						return true;
					}
				} )(),
...
function createConfigFile( data ) {
	...
	function getTemplate() {
		return new NPromise( function ( fulfill, reject ) {
			var templateFile = path.normalize( path.join( __dirname, '../../templates/wp-content.php.tmpl' ) );
	...
	function writeTemplate( args ) {
		var template = args[0],
			salts = args[1],
			configFileLocation = path.join( data.project, 'wp-config.php' ),
...
function runImport( args ) {
	return new NPromise( function ( fulfill ) {
		...
		NPromise.all( [
			...
			installVendorResources( data.vendor_resources, data.wordpress, data.cache ),
	...
	function compileLockData() {
		...
		return new NPromise( function ( fulfill ) {
			var lockData = {
					...
					wordpressPath    : '_cache/wordpress/' + data.wordpress,
					php              : data.php,
					resourceLocations: {
						'wp-config.php'     : windowCleaner( path.relative( vm.root, path.join( data.project, 'wp-config.php' ) ) ),
			...
			for ( i = 0; i < data.dev_resources.length; i += 1 ) {
				...
				key = windowCleaner( path.join( 'wp-content', resource.path ) );
...
function writeCompiledData( args ) {
	return new NPromise( function ( fulfill ) {
		NPromise.all( [createLockFile( args ), storeHostsMap( args )] )
			.then( function () {
				fulfill( args );
			}, _exit_error );
	} );
 ...
 */
var import_file = module.exports.import_file = function( config ) {
	if ( undefined === config ) {
 ...
 */
var import_json = module.exports.import_json = function( json ) {
 ...
 * Route the command
 */
app.cmd( /import\s?([^\s]+)?/, import_file );
