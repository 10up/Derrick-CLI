		...
		try {
			for ( x = 0; x < rawResources.length; x += 1 ) {
				var resource = resources.VcsResource.newFromVcs( rawResources[x] ),
					pathName = path.dirname( path.join( project, 'src', resource.path ) );
				...
				if ( fs.existsSync( pathName ) ) {
					...
					resourcePromises.push( resource.install( path.join( project, 'src', resource.path ) ) );
				}
			}
		} catch ( e ) {
			_exit_error( e );
...
function installVendorResources( vendorResources, wpVersion, project ) {
	return new NPromise( function ( fulfill, reject ) {
		...
		try {
			for ( x = 0; x < vendorResources.length; x += 1 ) {
				var resource = resources.Resource.newFromData( vendorResources[x] ),
					...
					resourcePath = path.join( project, resource.path );
		...
		resourcePromises.push( composer.install( composerManifest, project ) );
	...
	function compileLockData() {
		...
		return new NPromise( function ( fulfill ) {
			...
			for ( i = 0; i < data.dev_resources.length; i += 1 ) {
				...
				lockData.resourceLocations[key] = windowCleaner( path.relative( vm.root, path.join( data.project, 'src', resource.path ) ) );
			}

			for ( i = 0; i < data.vendor_resources.length; i += 1 ) {
				resource = resources.Resource.newFromData( data.vendor_resources[i] );
				key = windowCleaner( path.join( 'wp-content', resource.path ) );
				lockData.resourceLocations[key] = windowCleaner( path.relative( vm.root, path.join( data.project, 'vendor', resource.path ) ) );
			}
