...
function createConfigFile( data ) {
	...
	var request = require( 'request' ),
		...
		templateContents,
		templateFile = path.normalize( path.join( __dirname, '../../templates/wp-content.php.tmpl' ) ),
		templateData = {
			db   : {
				name: data.name,
				user: data.name,
	...
	function writeTemplate() {
		return new NPromise( function ( fulfill, reject ) {
			var config = fs.createWriteStream( configFileLocation, {flags: 'w+'} ),
				configContents = Mustache.render( templateContents, templateData );
