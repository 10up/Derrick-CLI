 ...
 *
 * @param {String} data Object to write
 * @param {String} file Filename of target storage
 *
 * @returns {NPromise}
 */
...
function createProjectDirectories( args ) {
	...
	return new NPromise( function ( fulfill ) {
		...
		debug( 'Creating project directories' );
		...
		mkdir( project )
			...
			NPromise.all( [
				...
				mkdir( path.join( project, 'src/config' ) )
			] ) )
			.then( function () {
				fulfill( [vm, data] );
			}, _exit_error );
...
function installDevResources( rawResources, project ) {
	return new NPromise( function ( fulfill, reject ) {
		...
		debug( 'Installing dev resources' );
 ...
 * @returns {*}
 */
function installVendorResources( vendorResources, wpVersion, project, data ) {
	return new NPromise( function ( fulfill, reject ) {
		debug( 'Installing vendor resources' );

		// Make sure some app params are configured
		if ( _.isEmpty( app.argv[ 'project' ] ) ) {
			app.argv.project = data.name;
		}
...
function createDatabase( name ) {
	return new NPromise( function ( resolve, reject ) {
		...
		try {
			var Command_CreateDB = require( '../commands/create-db' ),
				c = new Command_CreateDB();

			c.run( name, name, name ).then( resolve, reject );
...
/**
 * Import the database dump from a backup.
 *
 * @param {Array} args
 *
 * @return {NPromise}
 */
function reimportDatabase( args ) {
	var data = args[1],
		db_backup = path.join( 'sys', 'backups', data.name + '.sql' );

	return new NPromise( function( fulfill, reject ) {
		debug( 'Importing database backup.' );

		try {
			return commands.import_db( data.name, db_backup )
				.then( function() {
					fulfill( args );
				} );
		} catch( e ) {
...
function createAutoloader( project ) {
	...
	debug( 'Creating autoloader: ' + project );
...
function createConfigFile( data ) {
	...
	debug( 'Creating config file: ' + data );
	...
	function writeTemplate( args ) {
		var template = args[0],
			salts = args[1],
			configFileLocation = path.join( data.project, 'src', 'config', 'wp-config.php' ),
...
function createNginxConfigs( data ) {
	...
	debug( 'Create nginx configs: ' + data );
...
/**
 * Mount the overlaid filesystem on the server.
 *
 * This takes the following project structure:
 * /srv
 * .. /projects
 * .. .. /[project]
 * .. .. .. /src
 ...
 * .. .. /themes
 *
 * This can then be easily mounted within the files Docker container
 *
 * @param {String} shortname
 *
 * @returns {NPromise}
 */
function mountFilesystem( shortname ) {
	var command_params = {
	...
	return new NPromise( function( fulfill, reject ) {
		...
		var com = command( 'mount-fs', command_params );
...
function runImport( args ) {
	return new NPromise( function ( fulfill ) {
		...
		if ( data.dev_resources.length > 0 ) {
			tasks.push( installDevResources( data.dev_resources, data.project ) );
		}

		tasks.push( installVendorResources( data.vendor_resources, data.wordpress, data.project, data ) );
		tasks.push( createAutoloader( data.project ) );
		tasks.push( createDatabase( data.name ) );
		tasks.push( createConfigFile( data ) );
		tasks.push( createNginxConfigs( data ) );

		NPromise.all( tasks ).then( function() {
			return mountFilesystem( data.name );
		} ).then( function() {
			fulfill( [vm, data] );
		}, _exit_error );
	} );
...
/**
 * Run an abbreviated import chain.
 *
 * @param {Array} args
 * @returns {NPromise}
 */
function shortImport( args ) {
	return new NPromise( function( fulfill ) {
		var vm = args[0],
			data = args[1];
...
function createLockFile( args ) {
	...
	debug( 'Creating composer lock file' );
	...
	function compileLockData() {
		...
		return new NPromise( function ( fulfill ) {
			var lockData = {
					...
					overlayLocations: {
						'wp-content/themes': windowCleaner( path.join( '/fs', data.name, 'themes' ) ),
						'wp-content/plugins': windowCleaner( path.join( '/fs', data.name, 'plugins' ) ),
						'wp-content/mu-plugins': windowCleaner( path.join( '/fs', data.name, 'mu-plugins' ) ),
						'wp-content/config': windowCleaner( path.join( '/fs', data.name, 'config' ) )
					}
				},
				i,
				key,
...
function createAliases( args ) {
	...
	return NPromise.all( [getTemplate( 'phpunit.bat' ), getTemplate( 'phpunit' ), getTemplate( 'wp.bat' ), getTemplate( 'wp' ), getTemplate( 'base-config.php' )] )
		.then( writeTemplates );
}
 ...
 * @returns {NPromise}
 */
function shortWriteCompiledData( args ) {
	return new NPromise( function ( fulfill ) {
		NPromise.all( [
			storeHostsMap( args )
		] )
...
module.exports.import_file = function( config ) {
	...
	return resolve( config )
		...
		.then( settings.update_status, _exit_error )
...
/**
 * Re-import a specific project.
 *
 * @param {String} project
 */
module.exports.reimport = function( project ) {
	var config = path.join( 'projects', project, 'manifest.json' );

	return resolve( config )
		.then( parseJSON, _exit_error )
		.then( parseConfig, _exit_error )
		.then( settings.update_status, _exit_error )
		.then( shortImport, _exit_error )
		.then( reimportDatabase, _exit_error )
		.then( shortWriteCompiledData, _exit_error )
		.then( create_chain, _exit_error )
		.then( update_hosts, _exit_error )
		.then( resync_hosts, _exit_error )
		.then( function () {
			print.log( 'info', 'Finished reimporting %s', project );
...
/**
 * Grab a list of projects and reimport each in turn.
 */
module.exports.reimport_all = function() {
	...
	var projects = getDirectories( 'projects' );

	for ( var i = 0, l = projects.length; i < l; i++ ) {
		module.exports.reimport( projects[ i ] );
	}
};
...
/**
 * Route a secondary batch command.
 */
app.cmd( /reimport-all/, module.exports.reimport_all );

/**
 * Route a secondary command.
 */
app.cmd( /reimport\s?([^\s]+)?/, module.exports.reimport );