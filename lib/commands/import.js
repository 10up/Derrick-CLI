...
function moveManifestFile( data ) {
	...
	return new NPromise( function ( fulfill, reject ) {
		if ( !configFile ) {
			reject( 'Could not move manifest file into project directory' );
		}
		var targetLocation = path.join( data.project, 'manifest.json' );
		if ( configFile === targetLocation ) {
...
function importDatabase( args ) {
	...
	return new NPromise( function ( fulfill, reject ) {
		try {
			require( '../commands' ).import_db( data.name, data.database ).then( fulfill, reject );
		} catch ( e ) {
			reject( e );
		}
	} );
