...
function createLockFile( args ) {
	...
	function compileLockData() {
		var lockData = {},
			...
			resource;
		for ( i in data ) {
			if ( data.hasOwnProperty( i ) && 'hostname' !== i ) {
				lockData[i] = data[i];
			}
		}
		lockData.projectPath = windowCleaner( path.relative( vm.root, data.project ) );
		lockData.host = data.hostname[0];
		lockData.hosts = data.hostname;
		lockData.resourceLocations = {};
		...
		for ( i = 0; i < data.vendor_resources.length; i += 1 ) {
			resource = resources.Resource.newFromData( data.dev_resources[i] );
		...
		return NPromise.resolve( lockData );
	}
	...
	function writeLockData( lockData ) {
		return new NPromise( function ( fulfill, reject ) {
			...
			lockFile.write( JSON.stringify( lockData ) );
		} );
	}

	return new NPromise( function ( fulfill, reject ) {
		return compileLockData()
			.then( writeLockData, reject )
			.then( fulfill, reject );
	} );
}
