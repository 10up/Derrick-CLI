...
PassthruCommand.prototype.parseDerrickArgs = function() {
	if ( ! app.argv.project) {
		...
		// Make sure the project is relative
	}
 ...
 * @returns {NPromise}
 */
PassthruCommand.prototype.command = function( path, rawArgs ) {
	var SELF = this;

	if ( ! SELF.parsed ) {
		SELF.parseDerrickArgs();
	}
	...
	return new NPromise( function ( fulfill, reject ) {
		...
		if ( 'composer' === SELF.name ) {
			cmdArgs.path = SELF.project;

			cmdArgs.rawArgs = rawArgs;
		}

		var connection = command( SELF.name, cmdArgs ),
			output = '';
