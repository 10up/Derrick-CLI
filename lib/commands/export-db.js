 ...
 * @returns {NPromise}
 */
function compressDatabase( args ) {
	...
	return new NPromise( function( fulfill ) {
		fs.readFile( dump_file, function( err, data ) {
			zlib.gzip( data, function( huh, buf ) {
				...
				fs.writeFile( 'projects/' + db_name + '-db-' + date + '.sql.gz', buf, function() {
					print.info( 'Finished exporting database to projects/' + db_name + '-db-' + date + '.sql.gz' );
