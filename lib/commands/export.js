...
module.exports = function( projectName ) {
	...
	function parseVendorResources( type ) {
		return new NPromise( function( fulfill ) {
			var commandArgs = [type, 'list', '--format=json'];
			if ( 'mu-plugin' === type ) {
				commandArgs = ['derrick', 'get-mu-plugins']
			} 

			commands.wp( 'projects/' + projectName, '', commandArgs).then( function( output ) {
				try {
				...
				resources.forEach( function( resource ) {
					...
					if ( vendorsByName[name] && type === vendorsByName[name].type ) {
						...
						// Update reference
						vendorsByName[name].reference = resource.version;
					} else {
						// If plugin does not exist in manifest

						if ( 'must-use' === resource.status ) {
							/**
							 * wp plugin list returns must-use plugins like this. However, we are going to process
							 * these later with our own CLI command so let's ignore.
							 */
							return;
						}
	...
	return parseVendorResources( 'plugin' )
		...
		.then( function() {
			return parseVendorResources( 'mu-plugin' );
		} )
		.then( writeManifest )
		.then( exportDatabase );
		//.then( exportUploads );
