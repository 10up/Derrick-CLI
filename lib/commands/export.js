...
module.exports = function( path ) {
	...
	function parseVendorResources( type ) {
		return new NPromise( function( fulfill ) {
			commands.wp( manifest.hostname[0], '', [type, 'list', '--format=json']).done( function( error, output ) {
				if ( error ) {
					_exit( error );
				}
	...
	function exportDatabase() {
		return new NPromise( function( fulfill ) {
			commands.wp( manifest.hostname[0], '', ['eval', "'echo DB_NAME;'"]).done( function( error, output ) {
				if ( error ) {
	...
	return parseVendorResources( 'plugin' )
		.then( function() { parseVendorResources( 'theme' ); } )
		.then( writeManifest )
		.then( exportDatabase )
		.then( exportUploads );
};
